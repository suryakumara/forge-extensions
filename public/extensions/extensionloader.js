...
$(document).ready(function () {
  ...
  function init(config) {
    ...
    document.addEventListener("loadextension", function (e) {
      ...
      loaderconfig.Viewer.loadExtension(e.detail.extension);
    ...
    document.addEventListener("viewerinstance", function (e) {
      ...
      if (
        config.InbuiltExtensionsConfig &&
        config.InbuiltExtensionsConfig.CreateList === "true"
      )
    ...
    function CreateList() {
      ...
      Extensions.forEach((element) => {
        if (element.includeinlist === "true") {
          ...
          if (element.editoptions === "true")
            editoptions =
              '&nbsp;<i class="fas fa-cog editoptions" data-index="' +
              index +
              '"  data-toggle="modal" data-target="#editConfigModal"></i>';
          ExtensionList +=
            '<label><input class="checkextension" type="checkbox"' +
            checked +
            ' name="' +
            name +
            '" value="' +
            ...
            '"> ' +
            element.displayname +
            '</label>&nbsp;<i class="fas fa-info-circle details" data-toggle="popover" ></i>' +
            editoptions +
            "<br>";
        }
        index++;
      ...
      for (var i = 0; i < checkbox.length; i++) {
        ...
        if (element.bloglink)
          moredetails =
            '<a target="_blank" href="' + element.bloglink + '">Learn more</a>';
        if (element.gif)
          gif =
            '<br><img src="./extensions/' +
            element.name +
            '/extension.gif" alt="Sample Image">';
        let contents =
          "<p>" + Extensions[index].description + "</p>" + moredetails + gif;
      ...
      function editextensionconfig(e) {
        ...
        document
          ...
          .setAttribute("href", element.bloglink);
      }
      document.getElementById("saveconfig").onclick = saveoptions;
      function saveoptions() {
        console.log(editoptionindex);
        Extensions[editoptionindex].options = JSON.parse(
          document.getElementById("editextensionconfig").value
        );
        loaderconfig.Viewer.unloadExtension(Extensions[editoptionindex].name);
        loaderconfig.Viewer.loadExtension(
          Extensions[editoptionindex].name,
          Extensions[editoptionindex].options
        );
      }
      function togglecustomextension(e) {
        ...
        if (e.target.checked) {
          loaderconfig.Viewer.loadExtension(
            e.target.value,
            Extensions[parseInt(this.dataset.index)].options
          );
        } else {
    ...
    function ListInbuiltExtensions() {
      ...
      for (let index = 0; index < Extensions.length; index++) {
        ...
        if (element.includeinlist !== "false") {
          ...
          ExtensionList +=
            '<label><input class="checkextensionbuiltin" type="checkbox"' +
            checked +
            ' name="' +
            element.name +
            '" value="' +
            element.name +
            '"> ' +
            element.name.slice(9, element.name.length) +
            "</label><br>";
        }
      }
      list.innerHTML = ExtensionList;
